...
const User = () => {
  ...
  const determineCurrentEvent = (time: Date) => {
    ...
    for (const [event, { startDate, endDate }] of Object.entries(
      eventSchedule
    )) {
      console.log(
        `Checking event: ${event} (${startDate.toISOString()} - ${endDate.toISOString()})`
      );
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-start bg-white pt-20 font-source text-black">
      ...
      <div className="w-2/3 rounded-xl bg-white p-10 shadow-lg md:w-1/2">
        <h1 className="mb-5 font-bebas text-5xl">
          Mark Your Attendance for {event || "Event"}
        </h1>
        <input
          type="text"
          ...
          className="mb-4 w-full rounded border border-blue-500 bg-white p-2"
        />
        <button
          onClick={handleButtonClick}
          className="w-full rounded bg-sky-700 px-4 py-2 font-semibold text-white hover:bg-sky-800"
        >
          Mark Attendance
        </button>
      </div>
